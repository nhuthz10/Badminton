...
const BlogPost = ({ params }) => {
  ...
  const fetchBlogDetail = async () => {
    try {
      ...
      let res = await handleGetBlogDetailService(params.id);
  ...
  useEffect(() => {
    if (data) {
      setValue("blogTitle", data.title);
      setValue("blogDes", data.description);
  ...
  const onSubmit = async (submitData) => {
    try {
      ...
      let res = await handleUpdateBlogService(formData);

      if (res && res.errCode === 0) {
        toast.success("Cập nhật bài viết thành thành công");
        setValue("blogTitle", "");
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="model-add-update">
      <h1 className="model-add-update-header">Sửa thông tin bài viết</h1>
