...
const ProductSizePost = ({ params }) => {
  ...
  const fetchProductSizeDetail = async () => {
    try {
      ...
      let res = await handleGetProductSizeDetailService(params.productSizeId);
      if (res && res.errCode === 0) {
        setData(res.data);
        await dispatch(
          fetchAllProductSizeOfTheProductTypeRedux(
            res.data?.ProductSizeData?.productTypeId
          )
        );
      }
    } catch (error) {
  ...
  const onSubmit = async (submitData) => {
    try {
      ...
      let res = await handleUpdateProductSizeService({
        ...
        productId: data.ProductSizeData.productId,
      ...
      if (res && res.errCode === 0) {
        ...
        router.push(`/product/${data.ProductSizeData.id}/productsize/`);
      }
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="model-add-update">
      <h1 className="model-add-update-header">
