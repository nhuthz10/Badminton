  ...
  let handleSendOpt = async (data) => {
    ...
    try {
      ...
      if (res && res.errCode === 0) {
        toast.success("Đổi mật khẩu thành công", { className: "toast-normal" });
        router.push(`/login`);
      }
    } catch (err) {
      if (err?.response?.data?.errCode === 2) {
        ...
        toast.error("Mã OTP của bạn đã hết hạn", { className: "toast-normal" });
      } else if (err?.response?.data?.errCode === 4) {
        toast.error("Người dùng không tồn tại", { className: "toast-normal" });
      } else if (err?.response?.data?.errCode === -4) {
