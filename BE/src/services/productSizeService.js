...
let getProductSizeDetailService = (id) => {
  return new Promise(async (resolve, reject) => {
    try {
      if (!id) {
        resolve({
          ...
          include: [
            {
              model: db.Product,
              as: "ProductSizeData",
              attributes: ["productId", "name", "productTypeId", "id"],
            },
            {
              model: db.Size,
              as: "SizeData",
        ...
        if (!productSize) {
          resolve({
            errCode: 2,
            message: "ProductSize isn't exist",
          });
        } else {
          resolve({
            errCode: 0,
            data: productSize,
            message: "Get productSize succeed",
          });
        }
      }
    } catch (error) {
