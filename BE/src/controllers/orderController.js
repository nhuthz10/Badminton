...
let handleCreatePaymentUrlVnPay = (req, res) => {
  try {
    ...
    let orderId = generateOrderIdFromUUID();
    ...
    if (secureHash === signed) {
      if (vnp_ResponseCode === "00" && vnp_TransactionStatus === "00") {
        ...
        if (message.errCode === 0) {
          let orderDetail = await orderService.getOrderDetailService(
            message?.orderId
          );
          global._io.emit("notify-order-succeed", orderDetail.data);
          return res.redirect(
            `${process.env.URL_CLIENT}/user/orders/${message?.orderId}?vnp_ResponseCode=${vnp_ResponseCode}`
          );
        } else return res.status(400).json(message);
      } else {
        return res.redirect(
          `${process.env.URL_CLIENT}/user/cart?vnp_ResponseCode=${vnp_ResponseCode}`
        );
      }
    }
  } catch (error) {
